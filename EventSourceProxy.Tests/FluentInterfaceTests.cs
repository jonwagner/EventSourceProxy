...
namespace EventSourceProxy.Tests
{
    ...
    public class FluentInterfaceTests
    {
        ...
        {
            For<IEmailer>.Method(source => source.Send(Any<IEmail>.Ignore)).With<IEmail>().Trace(email => email.From);
        }

        [Test]
        public void ForSourceWithMethodParamTraceAs()
        {
            For<IEmailer>.Method(source => source.Send(Any<IEmail>.Ignore)).With<IEmail>().Trace(email => email.From).As("From");
        }

        [Test]
        public void AnythingWithParamTrace()
        {
            Anything.With<IEmail>().Trace(email => email.From);
        }

        [Test]
        public void AnythingWithParamTraceAs()
        {
            Anything.With<IEmail>().Trace(email => email.From).As("From");
        }

        [Test]
        public void AnythingWithParamTraceTrace()
        {
            Anything.With<IEmail>().Trace(email => email.From).Trace(email => email.To);
        }

        [Test]
        public void AnythingWithParamTraceAsTraceAs()
        {
            Anything.With<IEmail>().Trace(email => email.From).As("From").Trace(email => email.To).As("To");
        }
    }
}
